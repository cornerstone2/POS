...
class AdminWindow(BoxLayout):
    ...
    def add_user_fields(self):
        ...
        self.add_user(crud_first.text, crud_last.text, crud_user.text, crud_pwd.text,
                      crud_des.text))
    ...
    def add_product_fields(self):
        ...
        crud_order = TextInput(hint_text='Order', multiline=False)
        crud_purchase = TextInput(hint_text='Product Last Purchase', multiline=False)
        crud_submit = Button(text='Add', size_hint_x=None, width=100, on_release=lambda x:
        self.add_product(crud_code.text, crud_name.text, crud_weight.text, crud_stock.text,
    ...
    def add_product(self, code, name, weight, stock, sold, order, purchase):
        ...
        values = [code, name, weight, stock, sold, order, purchase]  # [datetime.now()]#
    ...
    def update_user_fields(self):
        ...
        self.update_user(crud_first.text, crud_last.text, crud_user.text, crud_pwd.text,
                         crud_des.text))
    ...
    def update_product_fields(self):
        ...
        crud_purchase = TextInput(hint_text='Product Last Purchase', multiline=False)
        crud_submit = Button(text='Update', size_hint_x=None, width=100, on_release=lambda x:
        self.update_product(crud_code.text, crud_name.text, crud_weight.text, crud_stock.text,
                            crud_sold.text, crud_order.text, crud_purchase.text))
    ...
    def update_product(self, code, name, weight, stock, sold, order, purchase, pwd=None):
        ...
        if code == '':
            self.notify.add_widget(Label(text='[color=#FF0000[b]Code required[/b]'
                                              '[/color]', markup=True))
            self.notify.open()
            Clock.schedule_once(self.killswitch, 1)
        else:
            ...
            if target_code == None:
                self.notify.add_widget(Label(text='[color=#FF0000[b]Invalid Code[/b]'
                                                  '[/color]', markup=True))
                self.notify.open()
                Clock.schedule_once(self.killswitch, 1)
            else:
                ...
                self.add_products.update_one({'product_code': code}, {
                    '$set': {'product_code': code, 'product_name': name,
                             'product_weight': weight, 'in_stock': stock,
                             'sold': sold, 'order': order, 'last_purchase': purchase}})
    ...
    '''def get_users(self):
        client = MongoClient()
        db = client.CORNERSTONE-POS System
        users = db.users
        _users = OrderedDict()
        _users['first_names'] = {}
        _users['last_names'] = {}
